...
with_mock_api({
  ...
  test_that("test user type returned is accurate", {
    ...
    for (i in 1:length(user_list[["username"]])) {
      ...
      # test user type response
      user_to_expect <- unlist(user_list[["user_type"]][user])
      response <- getMyUserType()
      print(response)
      print(response == user_to_expect)
