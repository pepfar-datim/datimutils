...
with_mock_api({
  ...
  test_that("Get a datstore object ", {
    ...
    data <- getDataStoreKey("dataQualityTool", "settings", d2_session = play2363)
  ...
  test_that("Can error with missing namespace", {
    expect_error(getDataStoreKey(NA, "settings", d2_session = play2363))
  })

  test_that("Can error with missing key", {
    expect_error(getDataStoreKey("dataQualityTool", NA, d2_session = play2363))
  })
