...
Once a user type is classified, developers can start to enforce limitations on data access. Mechanisms can be pulled and filtered on the desired columns; here for example we extract only code and name:  
...
#show mechs by mechs id
  observeEvent(input$mech_id_button, {
    ...
    # test if a user is a partner
      ...
      my_cat_ops <- listMyMechs()
      my_cat_ops_f <- merge(my_cat_ops, sample_data, by = "code")
