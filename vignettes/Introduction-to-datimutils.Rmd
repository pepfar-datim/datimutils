...
Below you will see how datimutils can be used to log into a DHIS2 instance, retrieve metadata, and retrieve aggregated data for further analysis in R.
...
Before querying the DATIM api, you must log into a DATIM instance. There are three basic ways to do so with datimutils. All three utilize the `loginToDATIM` function. NOTE this function can log in to any DHIS2 instance, not just DATIM, base on the provided baseurl.
...
> Note: By default, this DHIS2 session will expire after 60 minutes. When it does, you will need to re-authenticate

### Method 1 - username and password parameters

The most basic method is to pass your username and password directly into the `loginToDATIM` function, along with the web address of the server you wish to log in to.
...
The other two methods require the use of a configuration file. Make sure that you remember the location of this config file, as you will need it to pass into the `loginToDATIM` function. If a user wants the ablility to quickly log into multiple different servers, they can have a config file for each one.

In this vignette, our config file is a JSON file called, *play.json*.

There are two ways set up this config file.
...
```
The function will look in your keyring/credential store for the password, and if not found, will prompt you to enter a password and will store it automatically under that username and service (baseurl) for the next login. A user may prefer storing their passwords in their operating systems credentiual store because it allows them to avoid storing passwords in plain text on their computer, which is a potential security risk.   
...
### Metadata helpers when you have a vector of identifiers (e.g. uids or names)

Datimutils provides a number of high level metadata helpers for obtaining details (properties) of the different types of metadata. There is one for each of the main metadata categories in DHIS2 and auto complete in your R IDE will often make typing these very fast. This is the list of the numerous high level metadata helpers in datimutils.
...
Note that you get a result for "Kenema" both times it appears. This is an advantage of datimutils. The metadata helpers will return matched results in the same order as the lookup vector passed to the function. Because of this, it works well with tidyverse functions like `dplyr::mutate`. Here we add a new column with data element names corresponding to the existing data element uid columns. 
...
It is also possible to look up metadata using a different identifier such as name or code, but you must specify the look up identifier for your input vector.
...
There is a analytics endpoint helper, `getAnalytics`. It takes arguments for dimensions and filters, and returns aggregated data. Here is an example:
...
In our sample case, the data element passed was the uuid for 'ANC 1 Coverage,' The organisation unit passed was the uuid for 'Bo,' and the period was the last 12 months.


### Dimensions vs filters

Adding '_f' to the end of a parameter in `getAnalytics` turns from from a dimension into a filter ('ou' to 'ou_f'). Because of this change, the following commands look similar, but have one key difference:
...
In the first command, because the 'ou' argument is a *dimension*, the `getAnalytics` function queries each of the uuids in the array individually, and meaning that the organisation unit appears as a column in the resulting dataframe. In the second command, because the 'ou_f' argument is a *filter*, the `getAnalytics` aggregates data from the array of uuids as a whole and doe snot include the organisation unit as a column in the resulting dataframe.
