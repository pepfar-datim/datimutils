...
Logging in can be done with the `loginToDATIM` function. It takes a file path that contains the login credentials to Datim. For example here is a play server file called `play.json`
...
There is a wrapper for every metadata endpoint within datimutils. Here are calls to dataElements and categoryOptionGroups for example:
